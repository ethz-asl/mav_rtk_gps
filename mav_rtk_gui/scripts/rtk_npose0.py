...
#
#  Title:        rtk_info.py
#  Description:  ROS module to create a GUI to check RTK status, reset Rovio and reset MSF.
#
...
from gui_frames.rovio_frame import RovioFrame
...
class RtkNpose0:
    def __init__(self):
        # Main tkinter window.
        self.root = Tk()
        self.root.wm_title("RTK & Rovio NPOSE0 & MSF")
        ...
        self.frame_1_1 = Frame(self.column_1)
        self.frame_1_1.pack(side = TOP)
        ...
        # Create reset rovio frame.
        self.reset_rovio_frame = self.create_reset_rovio_frame(self.frame_1_0)

        # Create MSF frame.
        self.msf_frame = self.create_msf_frame(self.frame_1_1)
    ...
    def create_reset_rovio_frame(self, parent_frame):
        reset_rovio_frame = RovioFrame(parent_frame)

        return reset_rovio_frame
...
if __name__ == '__main__':
    rospy.init_node('rtk_npose0')
    rospy.loginfo(rospy.get_name() + ' start')

    # Go to class functions that do all the heavy lifting. Do error checking.
    try:
        rtk_npose0_gui = RtkNpose0()
